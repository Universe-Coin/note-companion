...
| **Could not start** / `undefined is not a function` on white | Module eval (`loadModuleImplementation`). **Build 32** still dies after the Clerk native + URL-polyfill Metro change: throw at `132360:78`, parents `132182` → `131983` → `131953`. Lines only moved +7 vs build 31, so this is **not** `clerk.js`. Hermes production bundles are unminified (`minify` off when Hermes is on); stacks name Metro lines, not source files. | Do not rebuild the Clerk-only commit again. Next build must annotate the throwing module path (`metro-eval-annotate-transformer.js`). Then fix that file. |
