...
| **Could not start** / `undefined is not a function` on white | Module eval (`loadModuleImplementation`). Two causes: (1) Metro `require.resolve("@clerk/clerk-js")` picks DOM `clerk.js`; `platform` is not always `"ios"` so a platform===ios check still bundles the DOM file. (2) Clerk’s `react-native-url-polyfill/auto.js` runs a whatwg-url bundle that can throw the same TypeError in production Hermes. | Resolve `@clerk/clerk-js` to `clerk.native.js` whenever `platform !== "web"`. Shim `react-native-url-polyfill/auto` (RN 0.83 already has `URL`). extraNodeModules must point at real package roots, not `packages/mobile/node_modules/@clerk` |
