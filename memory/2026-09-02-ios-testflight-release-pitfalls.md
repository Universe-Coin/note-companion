...
| **Could not start** / `undefined is not a function` on white | Module eval. **33** = `expo-router` Toast (`StyleSheet.create` / bottom-tabs barrel). **34** = Toast shim worked; next throw is `createNativeStackNavigator.js` → StackClient. That file calls `isLiquidGlassAvailable()` at require-time; `expo-glass-effect` is not linked in this app (`syncPlugins: false`, not in plugins). | Shim Toast + css-interop `StyleSheet.create` + `expo-glass-effect` (`isLiquidGlassAvailable` → false). Keep path annotations. |
...
- `metro.config.js` — native `@clerk/clerk-js` → `dist/clerk.native.js`; shim expo-router Toast, css-interop `StyleSheet.create`, and `expo-glass-effect`
...
- **Do not** assume `undefined is not a function` at boot is Clerk — 33 was Toast/StyleSheet; 34 was `expo-glass-effect` (`isLiquidGlassAvailable()` at stack navigator load).
